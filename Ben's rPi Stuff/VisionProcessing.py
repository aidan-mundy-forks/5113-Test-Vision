    ...
    if len(filteredContours) < 2:
        ...
        for i in range(len(filteredContours[:-1])):
            ...
            for j in range(i+1, len(filteredContours)):
                ...
                # determines which target is left and which one is right
                ...
                ############### correctly sorts the contours into the following order ##################
                ...
                if ft.superStrictTest(warpedLeft, warpedRight, CONFIG['SUPER_STRICT_TEST_POINTS'], SUPER_STRICT_TEST_TOLERANCE):
                    ...
                    # implemented from here: https://dsp.stackexchange.com/questions/1484/how-to-compute-camera-pose-from-homography-matrix
                        #pose = 

                    #theta -= ANGLE_CALIBRATION
                    pose = utils.getAngle(h)
